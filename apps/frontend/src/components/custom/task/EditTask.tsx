...
const EditTaskSchema = z.object({
  title: z.string().trim().min(3).max(20).optional(),
...
const EditTask = (props: { open: boolean; close: () => void; listId: string; taskId: string }) => {
  ...
  const priorities = Object.keys(Priority).filter((priority) => !isNaN(parseInt(priority)));
  ...
  async function onSubmit(data: z.infer<typeof EditTaskSchema>) {
    ...
    if (request.data) {
      ...
      toast({
        ...
        description: (
          <p>
            Task <b>{data.title}</b> successfully updated.
          </p>
        ),
      });
            ...
            <div className="flex flex-col space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      ...
                      <Input
                        type="text"
                        defaultValue={retrieveTask.title}
              ...
              <div>
                <FormField
                  control={form.control}
                  name="priority"
                  defaultValue={Object.keys(Priority).indexOf(String(retrieveTask.priority))}
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      ...
                      <Popover open={openPrioritySelector} onOpenChange={setOpenPrioritySelector}>
                        <PopoverTrigger asChild>
                          <Button variant="outline" role="combobox" aria-expanded={open} className="justify-between">
                            {Priority[retrieveTask.priority]}
                            <IconSelector stroke={1.5} size={16} className="ml-2 shrink-0 opacity-50" />
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="max-w-md p-0">
                          <Command>
                            ...
                            <CommandGroup>
                              {priorities.map((priority) => (
                                <CommandItem
                                  key={priority}
                                  value={Priority[parseInt(priority)]}
                                  onSelect={() => {
                                    field.onChange(Number(priority));
                                ...
                                >
                                  <IconCheck
                                    ...
                                    className={cn(
                                      "mr-2",
                                      Number(field.value) === Number(priority) ? "opacity-100" : "opacity-0",
                                    )}
                                  />
